  ...
  return (
    <section className="max-w-[1400px] mx-auto px-4">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-auto lg:h-[600px]">
        ...
        <div className="lg:col-span-8 group relative overflow-hidden rounded-[2rem] bg-slate-900 border border-slate-800 shadow-2xl">
          <Swiper
            modules={[Autoplay, Pagination, Navigation, EffectFade]}
            effect="fade"
            fadeEffect={{ crossFade: true }}
            speed={1200}
            loop={featuredSliderProducts.length > 1}
        ...
        <div className="lg:col-span-4 flex flex-col gap-6">
          ...
          <div className="flex-1 relative overflow-hidden rounded-[2rem] bg-slate-950 shadow-2xl group border border-slate-800">
              ...
              <div className="p-8 relative z-20 flex-1">
                <div className="flex items-center gap-2 mb-4">
                  <div className="relative">
                    ...
                    <span className="relative bg-blue-600 text-white text-[8px] font-black uppercase tracking-[0.2em] px-3 py-1 rounded-full border border-blue-400/30 flex items-center gap-2 shadow-lg shadow-blue-500/20">
                      <Clock size={10} className="opacity-80" /> Limited Time
                    </span>
                  </div>
                </div>

                <h3 className="text-xl lg:text-2xl font-black text-white mb-1 tracking-tighter leading-tight drop-shadow-2xl">
                  {promoProduct.name}
                </h3>
                <p className="text-blue-400 font-black text-[10px] mb-6 tracking-widest uppercase opacity-80">Flash Sale Event</p>

                <div className="flex gap-3">
                  {[
                    { val: timeLeft.hrs, label: 'H' },
                    { val: timeLeft.min, label: 'M' },
                    { val: timeLeft.sec, label: 'S' }
                  ].map((unit, i) => (
                    <div key={i} className="bg-slate-900/80 backdrop-blur-2xl border border-white/5 rounded-xl p-3 text-white text-center min-w-[50px] shadow-2xl group-hover:border-blue-500/30 transition-colors">
                      <p className="text-lg font-black tracking-tight text-blue-50">{unit.val}</p>
                      <p className="text-[7px] uppercase font-black tracking-widest text-blue-400/60 mt-0.5">{unit.label}</p>
                    </div>
                  ))}
                </div>
              </div>

              <div className="w-[40%] h-full relative overflow-hidden hidden sm:flex items-center justify-center p-4">
                <div className="absolute inset-0 bg-gradient-to-l from-blue-600/20 to-transparent"></div>
                {/* Floating Glow Orb */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 bg-blue-600/30 rounded-full blur-[80px] animate-pulse"></div>
                {promoProduct.image ? (
                  <img
                    src={promoProduct.image}
                    alt={promoProduct.name}
                    className="relative z-10 w-full h-[85%] object-contain transition-transform duration-1000 group-hover:scale-110 group-hover:rotate-3 drop-shadow-[0_20px_40px_rgba(0,0,0,0.8)]"
                  />
                ) : (
          ...
          {/* Bottom Square Banner - High-End Layout */}
          <div className="flex-1 relative overflow-hidden rounded-[2rem] bg-slate-950 shadow-2xl group border border-slate-800">
