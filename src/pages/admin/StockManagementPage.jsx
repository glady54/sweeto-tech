...
const Backdrop = ({ children, onClose }) => (
  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
    <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto border border-gray-100 dark:border-slate-800" onClick={e => e.stopPropagation()}>
      {children}
    </div>
  </div>
);
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
            ...
            <div className="flex items-center space-x-3">
              ...
              <button onClick={() => { resetForm(); setShowAddModal(true); }}
                className="flex items-center px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-blue-500/30 active:scale-95">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 relative">
          ...
          <input type="text" id="stock-search" name="search" placeholder={t('searchProducts')} value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
            className="w-full pl-12 pr-4 py-3.5 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl text-sm font-medium text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all" />
        </div>
                ...
                <tbody>
                  {filteredProducts.map(p => (
                    <tr key={p.id} className={`border-b border-gray-50 dark:border-slate-800/50 hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors ${isLowStock(p) ? 'bg-amber-50/50 dark:bg-amber-900/5' : ''}`}>
                      ...
                      <td className="px-6 py-4 text-right text-sm font-bold text-blue-600 dark:text-blue-400 font-mono">{formatPrice(p.price)}</td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end space-x-1">
                          ...
                          <button onClick={() => openEdit(p)}
                            className="p-2 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title={t('edit')}>
                            <Edit size={16} />
                          </button>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" id="add-product-name" name="name" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})}
                  className="w-full px-4 py-3 bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 rounded-xl text-sm text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              ...
              <div className="flex space-x-3 pt-4">
                ...
                <button onClick={handleAddProduct}
                  className="flex-1 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-bold transition-all shadow-lg shadow-blue-500/30">{t('saveProduct')}</button>
              </div>
            </div>
          </div>
        </Backdrop>
            ...
            <div className="space-y-4">
              ...
              <div className="flex space-x-3 pt-4">
                ...
                <button onClick={handleEditProduct}
                  className="flex-1 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-bold transition-all shadow-lg shadow-blue-500/30">{t('updateProduct')}</button>
              </div>
            </div>
          </div>
        </Backdrop>
            ...
            <div className="space-y-4">
              ...
              <div className="flex space-x-3 pt-4">
                ...
                <button onClick={handleAdjustStock}
                  className="flex-1 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-bold transition-all shadow-lg shadow-blue-500/30">{t('confirm')}</button>
              </div>
            </div>
          </div>
        </Backdrop>
